...
int fips_aes_ctx_init(struct cryptodev_ctx* ctx, int cfd, const uint8_t *key,
			 unsigned int key_size)
{
	...
	ctx->sess.cipher = CRYPTO_FIPS_AES_CBC;
...
void fips_aes_ctx_deinit(struct cryptodev_ctx* ctx)
{
...
int fips_aes_encrypt(struct cryptodev_ctx* ctx, const void* iv,
...
int fips_aes_decrypt(struct cryptodev_ctx* ctx, const void* iv,
...
static int test_fipsaes(int cfd)
{
	char plaintext1_raw[AES_BLOCK_SIZE + 63], *plaintext1;
	char ciphertext1[AES_BLOCK_SIZE]
	...
	fips_aes_ctx_init(&ctx, cfd, key1, sizeof(key1));
	...
	fips_aes_encrypt(&ctx, iv1, plaintext1, plaintext1, AES_BLOCK_SIZE);
	...
	fips_aes_ctx_init(&ctx, cfd, key2, sizeof(key2));
	...
	/* Encrypt data.in to data.encrypted */
	fips_aes_encrypt(&ctx, iv2, plaintext2, plaintext2, AES_BLOCK_SIZE);
